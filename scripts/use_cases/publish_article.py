...
from utils.models import FacebookPostResult, ZaloPostResult, PublishResult, BufferPostResult, GoogleBusinessPostResult, ParsedRequest
...
logger = get_logger("publish_article")
...
def _continue_publish(
    ...
    if should_facebook and "facebook" not in _active_for_gemini:
        _active_for_gemini.append("facebook")
    if should_google_business and "google_business" not in _active_for_gemini:
        _active_for_gemini.append("google_business")
    gemini_platforms: list[str] | None = _active_for_gemini or None

    need_captions = should_buffer or should_facebook or should_google_business
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        if should_google_business:
            gb_cap = social_captions.get("google_business")
            gb_text = gb_cap if gb_cap else (seo_summary if seo_summary else drive_article.title)
